...
const EditArticle = () => {
  ...
  const fetchArticle = useCallback(async () => {
    if (id) {
      try {
        ...
        const response = await axios.get(`http://localhost:5003/api/articles/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const fetchedArticle = response.data;
        setArticle({
          ...fetchedArticle,
          publishDate: dayjs(fetchedArticle.publishDate),
  ...
  }, [fetchArticle, fetchCategoriesAndUsers]);
  ...
  const handleUpdateArticle = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5003/api/articles/${id}`, updatedArticle, {
        headers: {
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <Button onClick={handleUpdateArticle} variant="contained" color="primary" fullWidth>
                  Update Article
                </Button>
              </Grid>
            </Grid>
